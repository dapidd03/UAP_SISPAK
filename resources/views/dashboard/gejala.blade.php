                    ...
                    <tbody>
                        @foreach ($gejala as $item)
                            <tr>
                                ...
                                <td>
                                    <span class="gejala-name-{{ $item->id }}">{{ $item->name }}</span>
                                    <form class="form-gejala-{{ $item->id }}" style="display: none;" method="post" action="{{ route('gejala.update', $item->id) }}">
                                        @csrf
                                        @method('patch')
                                        <div class="input-group col-md-12">
                                            <input type="text" name="gejala" class="form-control" value="{{ $item->name }}">
                                ...
                                @if (Auth::user())
                                    <td align="right">
                                        <button class="btn btn-primary btn-sm" onclick="toggleForm({{ $item->id }});">
                                            <i class="fa fa-edit"></i>
                                        </button>
                                        <button type="button" class="btn btn-danger btn-sm" onclick="deleteGejala({{ $item->id }});">
                                            <i class="fa fa-trash"></i>
                                        </button>
                                        <form class="delete-form-{{ $item->id }}" method="post" action="{{ route('gejala.destroy', $item->id) }}">
                                            @csrf
                                            @method('delete')
                                        </form>
                                    </td>
